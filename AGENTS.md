...
5. 作る前に`rg`等で既存の責務・呼出元・検査を探す。仕様・教訓・workflowは作業に必要な箇所だけ取得し、下位ディレクトリのAGENTSと必須gateを確認する。安全条件を読取削減のために省かない。
...
既存ファイルから対象を特定する。ホスティング、秘密情報、外部接続の有無を推測しない。入口の追加を製品の稼働確認としない。
